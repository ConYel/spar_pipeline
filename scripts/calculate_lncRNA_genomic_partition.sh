...
## calculate_lncRNA_genomic_partition.sh
## alex amlie-wolf
## overlaps SPAR intervals with lncRNA-related annotations
...
if [ $# == 3 ]; then
	...
	${BEDTOOLS} intersect -s -sorted -a stdin -b ${ANNOTATIONF} -wao -f 0.51 > ${WDIR}/Genomewide_distribution_patterns_of_small_RNA_loci_lncRNA_annot_overlaps.txt
	...
	${BEDTOOLS} intersect -s -sorted -a stdin -b ${ANNOTATIONF} -wao -f 0.51 > ${WDIR}/Genomewide_distribution_patterns_of_small_RNA_loci_lncRNA_unannot_overlaps.txt

    ## call the R script to make the table with partition assignments
    ${RSCRIPT} `dirname $0`/generate_lncRNA_partition_tables.R ${FPREFIX} ${WDIR}
else
