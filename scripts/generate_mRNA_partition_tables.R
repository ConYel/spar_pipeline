...
annot_partition_data <- read.table(paste0(wdir, "/", BASENAME, "_mRNA_annot_overlaps.txt"), header=F, sep="\t", quote="", as.is=T)
unannot_partition_data <- read.table(paste0(wdir, "/", BASENAME, "_mRNA_unannot_overlaps.txt"), header=F, sep="\t", quote="", as.is=T)
...
annot_partition_data$partition <- ifelse(annot_partition_data$partition=="5utr_exon", "5' UTR Exon",
                                 ...
                                 ifelse(annot_partition_data$partition=="intron", "mRNA Intron", "Intergenic")))))))
...
unannot_partition_data$partition <- ifelse(unannot_partition_data$partition=="5utr_exon", "5' UTR Exon",
                                 ...
                                 ifelse(unannot_partition_data$partition=="intron", "mRNA Intron", "Intergenic")))))))

## now write this out as extra columns on the original file, and also include the
## overlapping partition and the number of base pairs of overlap
write.table(cbind(unannot_locus_data, genomicPartition=unannot_partition_data$partition, overlapIds=unannot_partition_data$overlap_id, partitionChr=unannot_partition_data[,7], partitionStart=unannot_partition_data[,8], partitionEnd=unannot_partition_data[,9], partitionStrand=unannot_partition_data[,12], partitionBpOverlap=unannot_partition_data[,13]), paste0(wdir, "/", BASENAME, "_unannot_mRNA_genomic_partition.txt"), quote=F, sep="\t", row.names=F, col.names=T)
...
write.table(full_prop_tab, paste0(wdir, "/", BASENAME, "_allclass_mRNA_partition_props.txt"), quote=F, sep="\t", row.names=F, col.names=T)
...
full_mRNA_class_prop_tab <- rbind(class_prop_tab,
...
full_mRNA_class_prop_tab$class <- factor(full_mRNA_class_prop_tab$class,
                               levels=c(unique_classes, "Total"))

save(full_mRNA_class_prop_tab, file=paste0(wdir, "/", BASENAME, "_mRNA_partition_props_byclass.Rdata"))
# save.image(file=paste0(wdir, "/", BASENAME, "_all_data.Rdata")

write.table(full_mRNA_class_prop_tab, paste0(wdir, "/", BASENAME, "_mRNA_partition_props_byclass.txt"), quote=F, sep="\t", row.names=F, col.names=T)
