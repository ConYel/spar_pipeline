...
minPeakHeight=${minCoverage:-10}
minPeakLength=${minReadLength:-14}
minFoldChange=${minFoldChange:-2}
maxPeakLength=${maxReadLength:-44}
...
if [ "${strand}" == "neg"  ]; then
  ...
  revBg="'BEGIN{OFS=\"\t\"}{chr = \$1; chrStart=\$2; chrEnd=\$3; val=\$4; print chr, -(chrEnd-1), -(chrStart-1), val;}' | sort -k1,1 -k2,2n -k3,3n"
  ...
  #postprocessBedgraph="awk 'BEGIN{OFS=\"\t\"}{chrStart=-\$3; chrEnd=-\$2; \$3=chrEnd+1; \$2=chrStart+1; print \$0}' | sort -k1,1 -k2,2n -k3,3n";
  postprocessBedgraph="awk 'BEGIN{OFS=\"\t\"}{chrStart=-\$3; chrEnd=-\$2; \$3=chrEnd+1; \$2=chrStart+1; print \$0}' | sort -k1,1 -k2,2n -k3,3n";
fi
...
awk 'function computeEntropy(ent, cntArray)
     ...
     BEGIN{OFS="\t"; chr="";minHeight='${minPeakHeight}'+0; minRun='${minPeakLength}'+0; maxRun='${maxPeakLength}'+0; minFoldChange='${minFoldChange}'+0; peakCnt=0;strand="+"; if ("'${strand}'"=="neg") strand="-"; val_prev=0; verbose=0; peakFoldChange=0}
     {
        val = $4;
        #print "BG",$0;
        if (chr!=$1 || ($2>prev_end)) # next chromosome or gap (start > previous end)
        {
            ...
            if (inPeak==1)
            {
              ...
              b = prev_end;
              #if ((b-a+1) >= minRun)
              if ( ((b-a+1) >= minRun) && ((b-a+1) <= maxRun) )
              {
                exprVal-=beforePeakReadCnt;
                peakCnt+=1;
                peakID=("P" peakCnt);
               ...
               #print chr,a,b,peakID,exprVal,strand,entropy5p,normEntropy5p,maxEntropy5p, entropy3p, normEntropy3p, maxEntropy3p, maxProb5p, maxProb3p, maxPosition5p, maxPosition3p;
               print chr,a,b,peakID,exprVal,strand,entropy5p,normEntropy5p,maxEntropy5p, entropy3p, normEntropy3p, maxEntropy3p, maxProb5p, maxProb3p, maxPosition5p, maxPosition3p, beforePeakReadCnt, peakFoldChange;
               #print "before="beforePeakReadCnt;
               if (verbose==1)
               {
               len = b-a+1+0.0
               for (p in cnt5p)
        ...
        if (val_prev>0)
          fc = val/val_prev;
        # drop fold change should probably be a separate parameter
        # set to 2 for now
        if ( (val < minHeight) || (fc <= 1/2))
        ...
        #if ( val < minHeight )
        {
           ...
           if (inPeak==1)
           {
             ...
             if ( ((b-a+1) >= minRun) && ((b-a+1) <= maxRun) )
             {
               ...
               cnt3p[b-a+1-1]+=val_prev-val; # [) interval
               computeEntropy(ent, cnt3p);
               entropy3p = ent[1];
               normEntropy3p = ent[3];
               maxEntropy3p = ent[2];
        ...
        {  
           if (inPeak<0)
           {
             ...
             # if number of starting reads > min and fold change > min
             if ((fc >= minFoldChange) && ((val-val_prev) >= minHeight))
             {
                a=$2;
                inPeak = 1;
                #read coverage before the start
                beforePeakReadCnt=val_prev;
...
awk 'BEGIN{FS="\t";OFS="\t"}{print $1,$2,$3,$5}' ${INBG}.segm > ${INBG}.segm.bedGraph
